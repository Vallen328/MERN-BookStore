...
const AllOrders = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
        const response = await axios.get("https://d5vaxs5dc6.execute-api.ap-south-1.amazonaws.com/prod/api/v1/get-all-orders", { headers });
    ...
    <div key={i} className="bg-zinc-800 w-full rounded py-2 px-4 flex gap-4 hover:bg-zinc-900 hover:cursor-pointer">
      ...
      <div className="w-[40%] md:w-[22%]">
        {items.book ? (
          ...
          <h1 className="text-red-500">Book Not Available</h1>
        )}
      </div>
      <div className="w-0 md:w-[45%] hidden md:block">
        <h1 className="">{items.book ? items.book.desc.slice(0, 50) + " ..." : "No description available"}</h1>
      </div>
      <div className="w-[17%] md:w-[9%]">
