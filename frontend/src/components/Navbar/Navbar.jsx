...
const Navbar = () => {
    ...
    if(isLoggedIn === false){
        links.splice(2, 3);  //Beech 2nd index se dono kaatdo
    }
    if(isLoggedIn === true && role == "user"){
        links.splice(4, 1);  //Beech 2nd index se dono kaatdo
    }
    ...
    <>
        ...
        <div className={`${MobileNav} bg-zinc-800 h-screen absolute top-0 left-0 w-full z-40 flex flex-col items-center justify-center`}> 
            {links.map((items, i) => (
                ...
                key={i} onClick={() => MobileNav === "hidden"? setMobileNav("block"): setMobileNav("hidden")}>{items.title}{" "}
                </Link>
            ))}
